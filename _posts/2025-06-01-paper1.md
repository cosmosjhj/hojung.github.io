...
* **Align key eigenvalues**
  $\mathcal{L}_{\text{WA}}=\rho\sum_{j\in\text{occ}\cup\{\text{LUMO}\}}|\tilde H_{jj}-\epsilon^{\star}_{j}|^{2}+\xi\sum_{j\notin\text{occ}\cup\{\text{LUMO}\}}|\tilde H_{jj}-\epsilon^{\star}_{j}|^{2},\quad\rho\gg\xi.$  
